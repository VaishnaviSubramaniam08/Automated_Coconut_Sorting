...
while True:
    ...
    if len(boxes) == 0:
        ...
        for i, box in enumerate(boxes):
            ...
            if label.lower() in ["healthy", "good", "intact"]:  # adjust according to your healthy class
                weight = predict_coconut_weight(coconut_crop)
                weight_label = f"weight - {int(weight)} g" if weight is not None else "Weight N/A"
            ...
            cv2.putText(frame, f"{label}: {confidence:.2f}", (x1, max(y1-25, 20)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
            cv2.putText(frame, weight_label, (x1, max(y1-5, 20)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)

            # Console output
            print(f"Coconut {i+1}: Class={label}, Confidence={confidence:.2f}, Weight={weight_label}")

    cv2.imshow("Coconut Detection + Classification + Weight", frame)
